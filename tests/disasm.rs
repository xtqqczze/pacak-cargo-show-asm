#![cfg(all(feature = "disasm", not(target_env = "msvc")))]
...
fn disasm_poke() {
    ...
    assert!(
        out.contains("// tests/disasm-test-crate/src/lib.rs:3"),
        "{out}"
    );
    assert!(out.contains("pub fn disasm_test_function"), "{out}");
    assert!(out.contains("eprintln!(\"hello\");"), "{out}");
